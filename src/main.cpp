...
// Copyright (c) 2014 The Darkgold developers
...
unsigned int nTargetSpacing = 120; // 4 minute
...
const string strMessageMagic = "Darkgold Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Darkgold: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Darkgold: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Darkgold: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Darkgold fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Darkgold: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Darkgold: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Darkgold: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Darkgold: track money supply and mint amount info
...
// Darkgold: total coin age spent in transaction, in the unit of coin-days.
...
// Darkgold: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Darkgold: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Darkgold: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Darkgold: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Darkgold: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Darkgold: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Darkgold: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Darkgold: getblocks may not obtain the ancestor block rejected
    ...
    // Darkgold: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Darkgold", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "President Obama angers the whole of China by chewing gum Independent 12112014";
        CTransaction txNew;
        txNew.nTime = 1415798678;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415798678;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1361788;
        if(fTestNet)
        {
            block.nNonce   = 133765;
			block.nTime    = 1415798680;
			txNew.nTime = 1415798680;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("7a80b08f4fdf016c6681e4016f38c17beefb134994d7363afc59f78326610e75"));
        block.print();
        ...
        // Darkgold: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Darkgold: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Darkgold: tell downloading node about the latest block if it's
