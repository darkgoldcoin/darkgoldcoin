    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->DGDAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(BTC)"<< "DGD(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)"  << "DGD(SIZE)" << "TOTAL(BTC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(DGD)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //DGDst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-DGD");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-DGD&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-DGD&count=100");
      return Response;
}
...
QString tradingDialog::BuyDGD(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DGD&quantity=";
...
QString tradingDialog::SellDGD(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DGD";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=DGD";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DGD&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>DGD Volume:</b> <span style='font-weight:bold; font-size:11px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> DGD");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        ui->DGDSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:11px; color:blue'>" + str.number(DGDSupply,'i',8) + "</span><b> DGD</b>");
        ...
        ui->DGDDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:11px; color:blue'>" + str.number(DGDDemand,'i',8) + "</span><b> DGD</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance("DGD");
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->DGDAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance("DGD");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->DGDBalanceLabel,*ui->DGDAvailableLabel,*ui->DGDPendingLabel, QString::fromUtf8("DGD"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputDGD->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of BTC that can be gained from selling DGD available balance
    QString responseA = GetBalance("DGD");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableDGD = ResultObject["Available"].toDouble();

    ui->UnitsInputDGD->setText(str.number(AvailableDGD,'i',8));
}
...
void tradingDialog::on_BuyDGD_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyDGD(Order,Quantity,Rate);
...
void tradingDialog::on_SellDGDBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputDGD->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellDGD(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputDGD_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
